    ...
    <div class="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div class="flex flex-wrap justify-center gap-4 mb-12">
            ...
            @foreach($availableTypes as $type)
                <a href="{{ route('properties.index', ['type' => $type->property_type]) }}" class="group px-6 py-3 rounded-full border {{ request('type') === $type->property_type ? 'bg-secondary text-black border-secondary' : 'bg-black/30 text-white/80 border-white/20 hover:border-secondary hover:text-secondary' }} transition-all duration-300 backdrop-blur-sm">
                    <span class="font-medium">{{ $type->property_type }}</span>
                    <span class="ml-2 text-xs opacity-75">({{ $type->count }})</span>
                </a>
            @endforeach
